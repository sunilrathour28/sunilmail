...
if(isset($submit1))
{
    ...
    for($i=0;$i<$count; $i++){
        $del_id = $id[$i];
        $inbox1->parmanentDelete($del_id);
        if($inbox1){
            echo "<script>location.href='?page=trash';</script>";
        }
        else{
...
<table class="table table-striped table-hover" id="inbox_table">
    ...
    while ($arr1= mysqli_fetch_array($data1))
    {
        ...
        <tr>
            ...
            <td id="trash1"><strong><?php echo $arr1['sender']; ?></strong>
                <input type="hidden" id="hidden"  value="<?php echo $arr1['id']; ?>">
            </td>
            <td id="trash2"><?php echo "<span id='subject'>".$arr1['subject']."-</span>"."<span id='msg'>".substr($arr1['message'], 0, 30)."..."."</span>";  ?><input type="hidden" id="hidden"  value="<?php echo $arr1['id']; ?>"></td>
            <td id="trash3"><?php
                if($datediff < 60) {
